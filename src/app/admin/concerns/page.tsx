...
import { getAllConcerns, deleteConcern } from '@/modules/concerns/services/concern.service';
...
export default function AdminConcerns() {
  ...
  const { data: concernsResponse, isLoading, error } = useQuery({
    queryKey: ['concerns'],
    queryFn: () => getAllConcerns(),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => deleteConcern(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['concerns'] });
      toast.success('Concern deleted');
    },
    onError: (err: any) => {
      toast.error(err?.response?.data?.message || 'Failed to delete concern');
    }
  });

  const handleDelete = (id: number) => {
    if (confirm('Are you sure you want to delete this skin concern?')) {
  ...
  const filteredConcerns = concerns.filter((concern: Concern) => {
    const nameStr = removeAccents(concern?.name || '').toLowerCase();
    const slugStr = removeAccents(concern?.slug || '').toLowerCase();
        ...
        <main className="flex-1 overflow-y-auto p-8">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Manage Skin Concerns</h1>
                <p className="text-gray-600">Organize different skin concerns for your products.</p>
              </div>
              <button 
              ...
              >
                <Plus size={20} />
                <span className="font-semibold">Add Concerns</span>
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
               <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                   <p className="text-gray-500 text-sm font-medium">Total Concerns</p>
                  <p className="text-2xl font-bold text-gray-900 mt-1">{concerns.length}</p>
               </div>
            </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-gray-50/50">
                      ...
                      <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-widest">Concern Name</th>
                    ...
                    {isLoading ? (
                      ...
                      <tr>
                        <td colSpan={4} className="px-6 py-10 text-center text-red-500">
                          Error loading concerns. Please try again.
                        </td>
                      </tr>
                    ) : filteredConcerns.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="px-6 py-10 text-center text-gray-500">
                          {searchTerm ? `No concerns found matching "${searchTerm}"` : 'No concerns found.'}
                        </td>
                      </tr>
                    ) : (
                      filteredConcerns.map((concern: Concern) => (
                        <tr key={concern.id} className="hover:bg-gray-50/80 transition group">
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-4">
                              ...
                              <p className="font-bold text-gray-900 group-hover:text-[#7a9e8e] transition-colors">{concern.name}</p>
                            </div>
                          </td>
                          <td className="px-6 py-4 text-sm text-gray-500">
                            <code className="bg-gray-100 px-2 py-1 rounded text-xs font-mono">{concern.slug}</code>
                          </td>
                          <td className="px-6 py-4 text-right">
                            <div className="flex justify-end gap-2">
                               <button 
                                onClick={() => setEditingConcern(concern)}
