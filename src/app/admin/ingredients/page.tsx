...
import { getAllIngredients, deleteIngredient } from '@/modules/ingredients/services/ingredient.service';
...
export default function AdminIngredients() {
  ...
  const { data: ingredientsResponse, isLoading, error } = useQuery({
  ...
  const filteredIngredients = ingredients.filter((ingredient: Ingredient) => {
    const nameStr = removeAccents(ingredient?.name || '').toLowerCase();
    const slugStr = removeAccents(ingredient?.slug || '').toLowerCase();
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
              <div>
                ...
                <p className="text-gray-600">Organize active ingredients for your products.</p>
              </div>
              <button 
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
               <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                   <p className="text-gray-500 text-sm font-medium">Total Ingredients</p>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-gray-50/50">
                      ...
                      <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-widest">Ingredient Name</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {isLoading ? (
                      ...
                      <tr>
                        <td colSpan={4} className="px-6 py-10 text-center text-gray-500">
                          {searchTerm ? `No ingredients found matching "${searchTerm}"` : 'No ingredients found.'}
                        </td>
                      </tr>
                    ) : (
                      filteredIngredients.map((ingredient: Ingredient) => (
                        <tr key={ingredient.id} className="hover:bg-gray-50/80 transition group">
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-4">
                              ...
                              <p className="font-bold text-gray-900 group-hover:text-[#7a9e8e] transition-colors">{ingredient.name}</p>
                            </div>
                          </td>
                          <td className="px-6 py-4 text-sm text-gray-500">
                            <code className="bg-gray-100 px-2 py-1 rounded text-xs font-mono">{ingredient.slug}</code>
                          </td>
