...
import { createMultipleIngredients } from '../services/ingredient.service';
import { toast } from 'sonner';
import { motion, AnimatePresence } from 'framer-motion';

const ingredientsSchema = zod.object({
...
type IngredientsFormData = zod.infer<typeof ingredientsSchema>;
...
export function CreateIngredientModal({ isOpen, onClose }: CreateIngredientModalProps) {
  ...
  const mutation = useMutation({
    mutationFn: createMultipleIngredients,
    onSuccess: (response) => {
      if (response.success) {
        ...
        toast.error(response.message || 'Failed to create ingredients');
      }
    },
    onError: (error: any) => {
  ...
  const handleFormSubmit = (data: IngredientsFormData) => {
  ...
  const handleJsonSubmit = () => {
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        payload = parsed.ingredients;
      } else {
        throw new Error('JSON must be an array or an object containing an "ingredients" array.');
      }
      
      if (payload.length === 0) throw new Error('No items found');
      
      mutation.mutate({ ingredients: payload });
    } catch (err: any) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="flex items-center justify-between p-6 border-b border-gray-100 flex-shrink-0">
              <h2 className="text-xl font-bold text-gray-900">Create Ingredients</h2>
              <button 
                onClick={handleClose}
            ...
            <div className="overflow-y-auto p-6 flex-1">
              <form id="ingredient-form" onSubmit={submitHandler} className="space-y-4">
                {inputMode === 'json' ? (
...
["Salicylic Acid", "Niacinamide"]`}
                  ...
                  {fields.map((field, index) => (
                    <div key={field.id} className="flex gap-2 items-start">
                      <div className="flex-1 space-y-1.5">
                        <label className="text-sm font-semibold text-gray-700">Ingredient Name {index + 1}</label>
                        <input
                          ...
                          className={`w-full px-4 py-2.5 bg-gray-50 border rounded-xl focus:outline-none focus:ring-2 focus:ring-[#7a9e8e]/20 transition ${
                            errors.ingredients?.[index]?.name ? 'border-red-300 focus:border-red-500' : 'border-gray-200 focus:border-[#7a9e8e]'
                          }`}
                        />
                        {errors.ingredients?.[index]?.name && (
                          <p className="text-xs text-red-500 mt-1">{errors.ingredients[index]?.name?.message}</p>
                        )}
                      </div>
                ...
                {errors.ingredients && !Array.isArray(errors.ingredients) && (
                  <p className="text-xs text-red-500 mt-1">{errors.ingredients.message}</p>
                )}
                  </>
                )}
              </form>
            ...
            <div className="p-6 border-t border-gray-100 flex gap-3 flex-shrink-0 bg-white">
              ...
              >
                {mutation.isPending ? (
                  ...
                  `Create ${fields.length > 1 ? 'Ingredients' : 'Ingredient'}`
                )}
              </button>
            </div>
          </motion.div>
