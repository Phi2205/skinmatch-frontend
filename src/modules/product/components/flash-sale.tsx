...
export function FlashSale() {
  ...
  const { data: flashSaleResponse, isLoading } = useQuery({
    ...
    queryFn: () => getProductsByBadgeSlug('flash-sale', 1, 5), // Fetch 5 items to match the layout
  });
    ...
    <section className="py-12 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-[#7a9e8e] rounded-3xl p-5 sm:p-6 shadow-xl relative overflow-hidden">
          ...
          {/* Grid Layout (2 columns mobile, up to 5 columns on large screens) */}
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3.5">
            {isLoading ? (
              // Skeletal Loaders
              Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="bg-white rounded-2xl p-3 animate-pulse h-[340px] flex flex-col justify-between">
              ...
              <div className="col-span-full py-16 text-center text-white bg-[#6b8e7e]/50 rounded-2xl border border-white/20">
